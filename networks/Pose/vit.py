...
class VisionTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.flatten = nn.Sequential(
            nn.Linear(num_patches + 1, 1),
            nn.Flatten(),
        )

        # self.embeding_clas = nn.Embedding(10, embed_dim // 4, weight_attr=paddle.ParamAttr(regularizer=None, initializer=nn.initializer.TruncatedNormal()))
        # self.embeding_iid = nn.Embedding(8, embed_dim // 4, weight_attr=paddle.ParamAttr(regularizer=None, initializer=nn.initializer.TruncatedNormal()))
        # self.embeding_dis = nn.Linear(1, embed_dim // 2)
    ...
    #     x = self.flatten(paddle.transpose(x, [0, 2, 1]))
    #     c1 = self.embeding_clas(c1)
    #     c2 = self.embeding_iid(c2)
    #     c3 = self.embeding_dis(c3)
    #     x = x + self.merge(paddle.concat([c1, c2, c3], axis=1))
    #     x = self.head(x)
    #     return x
...
def BEiTv2_vit_base_patch16_224(config, **kwargs):
...
def BEiTv2_vit_large_patch16_224(config, **kwargs):
    model_name = sys._getframe().f_code.co_name
    model = VisionTransformer(
        model_name=model_name,
        img_size=224,
...
def MOCOV3_vit_small(config, **kwargs):
    """
    vit small in mocov3
    """
    model_name = sys._getframe().f_code.co_name
...
def MOCOV3_vit_base(config, **kwargs):
    """
    vit base in mocov3
    """
    model_name = sys._getframe().f_code.co_name
...
def MAE_vit_base_patch16(config, **kwargs):
    model_name = sys._getframe().f_code.co_name
    model = VisionTransformer(
        model_name=model_name,
        patch_size=16,
...
def MAE_vit_large_patch16(config, **kwargs):
    model_name = sys._getframe().f_code.co_name
    model = VisionTransformer(
        model_name=model_name,
        patch_size=16,
...
def MAE_vit_huge_patch14(config, **kwargs):
    model_name = sys._getframe().f_code.co_name
    model = VisionTransformer(
        model_name=model_name,
        patch_size=14,
...
def EVA_vit_giant_patch14(config, **kwargs):
    model_name = sys._getframe().f_code.co_name
    model = VisionTransformer(
        model_name=model_name,
        patch_size=14,
...
def CAE_vit_base_patch16_224(config, **kwargs):
    model_name = sys._getframe().f_code.co_name
    model = VisionTransformer(
        model_name=model_name,
        img_size=224,
